    ...
    fetch('projects/other/otherprojects.json')
        ...
        .then(data => {
            ...
            const generateProjectDetailsHTML = (project) => {
                ...
                const projectVideos = project.videos.map(video => {video.split(':')[0]==="https" ? `<iframe src="${video}" frameborder="0" allowfullscreen></iframe>` : `<video controls><source src="projects/other/videos/${video}" type="video/mp4"></video>`}).join('');
